---
title: "Critical Egocentric Mixed-Methods SNA: Analyzing Interviews with Women and/or Queer Ph.D. Physicists. Supplemental Material"
output: 
  pdf_document: 
...
Plotting egr objects using ´as_network()´ was used to produce Fig. 8 in the paper:
...
```{r pie charts of alter counts, fig.width=6}
...
# Plot pie charts
# Shows individual as example; group, institutional, and coll.superv use 
#  similar code.
par(mfrow = c(1, 2)) #how many plots to show per page
pie(individual[individual > 0], labels = indiv_lab, main = "individual", 
    cex = 0.8)
pie(total, labels = total_lab, main = "total", cex = 0.8)
par(mfrow = c(1, 1)) 
```
...
```{r pie charts of alter mentions, fig.width=6}
# Subset method 1: single interview
int <- 36
int_attr <- alter_attr %>% filter(interview == int)
...
par(mfrow = c(1, 2)) #how many plots to show per page
pie(individual_men[individual_men > 0], main = "Individual mentions", cex = 0.8)
pie(total_men[total_men > 0], main = "Total mentions", cex = 0.8)
par(mfrow = c(1, 1)) 
```

This can also be summed over a subset of interviews (here specified by interview number, but might be something like career sector):

```{r pie charts of aggregated mentions, fig.width=6}
# Summation for subset of interviews

int2_attr <- alter_attr %>% filter(interview %in% c(32, 36))

# Alter mentions of each type by category, also total
individual_men2 <- count_mentions(int2_attr, indiv_lab)
group_men2 <- count_mentions(int2_attr, group_lab)
institutional_men2 <- count_mentions(int2_attr, inst_lab)
coll.superv_men2 <- count_mentions(int2_attr, coll_lab)
total_men2 <- c(sum(individual_men2), sum(group_men2), sum(institutional_men2),
               sum(coll.superv_men2))
names(total_men2) <- total_lab

par(mfrow = c(1, 2)) #how many plots to show per page
pie(individual_men2[individual_men2 > 0], main = "Individual mentions", cex = 0.8)
pie(total_men2[total_men2 > 0], main = "Total mentions", cex = 0.8)
par(mfrow = c(1, 1)) 
```
